...
err3 = np.random.normal(loc=0.0, scale=.92, size=50)
...
removeaxes([ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9])
setaxlims([ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9], -1.5, 1.5)
...
y = -x - err
slope, intercept, r_value, p_value, std_err = stats.linregress(x, y)
y_fit1 = (slope * x) + intercept
#ax4.plot(x, y_fit1, '-', color='lightgray')
ax4.plot(x, y, '.')
...
slope, intercept, r_value, p_value, std_err = stats.linregress(x, y)
y_fit1 = (slope * x) + intercept
#ax7.plot(x, y_fit1, '-', color='lightgray')
ax7.plot(x, y, '.')
